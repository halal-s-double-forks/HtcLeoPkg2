...
if [ ${PAK} = apt ]; then
    sudo apt install -y pip git mono-devel build-essential nuget uuid-dev iasl nasm gcc-arm-none-eabi python3 python3-distutils python3-git python3-pip gettext locales gnupg ca-certificates python3-venv git git-core clang llvm curl||_error "\nFailed to install Packages!\n"
elif [ ${PAK} = dnf ]; then
    # TODO: Add ARM GCC for Fedora
    sudo dnf install -y git mono-devel nuget iasl nasm make gcc automake kernel-devel python3 python3-pip gettext gnupg ca-certificates git git-core clang llvm curl||_error "\nFailed to install Packages!\n"
    ...
    yay -Sy git mono base-devel nuget uuid iasl nasm arm-none-eabi-gcc python3 python python-distutils-extra python-git python-pip gettext gnupg ca-certificates python-virtualenv python-pipenv core-git clang llvm curl||_error "\nFailed to install Packages!\n"
else
...
export CLANG38_ARM_PREFIX=arm-none-eabi-